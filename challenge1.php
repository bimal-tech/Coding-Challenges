...
Mathematically, we can interpret this as follows: the binary representation of $n$ has only one set bit, which is the fourth bit 
from the right (corresponding to the digit 8 in decimal notation). When we subtract 1 from $n$, we get a binary number that has 
all the bits to the right of this set bit flipped, and the set bit itself cleared. In this case, the resulting binary number 
is 0111, which has no bits in common with the binary representation of $n$. Therefore, the bitwise AND operation between
$n$ and $n-1$ clears all the bits in $n$, leaving us with 0.
